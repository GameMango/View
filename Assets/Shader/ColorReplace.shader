Shader "Custom/ColorReplace"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            if (_REPLACE_COLOR_1 && length(abs(c.rgb - _TO_REPLACE_COLOR_1.rgb)) < 0.1)
            {
                c.rgb = _REPLACE_WITH_COLOR_1.rgb;
            } 
            else if (_REPLACE_COLOR_2 && length(abs(c.rgb - _TO_REPLACE_COLOR_2.rgb)) < 0.1)
            {
                c.rgb = _REPLACE_WITH_COLOR_2.rgb;
            } 
            else if (_REPLACE_COLOR_3 && length(abs(c.rgb - _TO_REPLACE_COLOR_3.rgb)) < 0.1)
            {
                c.rgb = _REPLACE_WITH_COLOR_3.rgb;
            } 
            else if (_REPLACE_COLOR_4 && length(abs(c.rgb - _TO_REPLACE_COLOR_4.rgb)) < 0.1)
            {
                c.rgb = _REPLACE_WITH_COLOR_4.rgb;
            } 
            else if (_REPLACE_COLOR_5 && length(abs(c.rgb - _TO_REPLACE_COLOR_5.rgb)) < 0.1)
            {
                c.rgb = _REPLACE_WITH_COLOR_5.rgb;
            }
            else if (_REPLACE_COLOR_6 && length(abs(c.rgb - _TO_REPLACE_COLOR_6.rgb)) < 0.1)
            {
